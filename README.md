...
- `NetCommon.h` : `PacketHeader` 구조체, `send_all()` / `recv_all()` 함수, `NetState` 구조체 포함
...
## 14. OOP / RAII 기반 구조 리팩토링 (진행 중)

현재 프로젝트는 단순한 절차형 Winsock 코드에서 벗어나
자원 중심(Resource-oriented) 구조를 가지도록
OOP / RAII 기반으로 리팩토링을 진행하고 있습니다.

리팩토링의 주요 목적은 다음과 같습니다.

* socket lifetime 자동 관리
* Winsock 초기화 / 정리 자동화
* listen socket / client socket 역할 분리
...
accept() 이후 생성되는 client socket을 관리하는 클래스입니다.
...
helper 함수와 연동하여 transport layer 책임을 캡슐화했습니다.

이 클래스 역시 socket ownership 중복을 방지하기 위해
...
현재 다음 구조까지 리팩토링이 완료되었습니다.

* WinsockGuard 적용 완료
* ListenSocket 적용 완료
* ClientSocket 적용 완료
* send_all / recv_all transport abstraction 유지
* PacketHeader 기반 protocol layer 유지

향후 main() 루프 구조 또한 객체 중심 구조로 단계적으로 리팩토링할 예정입니다.
